...
querySql = "select sid,icc_no,stb_no,cmd,bg_date,end_date,channel,mail,tune,networkid,product_name from cagw_queue_slave_test where rownum<=30 "
...
def ora_upd(ora, sid, status, result=None, ca_cmd=None, ca_ret=None):
    try:
        ...
        sql = "update cagw_queue_slave_test set status='%s'%s%s%s,update_date=sysdate where sid=%d" % (status, result_sql, cacmd_sql, caret_sql, int(sid))
        #print sql
        ora.execone(sql)
        return 1
    except Exception, msg:
