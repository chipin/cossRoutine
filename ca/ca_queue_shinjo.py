...
querySql = "select sid,icc_no,stb_no,cmd,decode(bg_date,null,to_char(sysdate-(1/3),'yyyymmdd'),to_char(bg_date-1,'yyyymmdd')) bg_date,decode(end_date,null,to_char(sysdate+3650,'yyyymmdd'),to_char(end_date,'yyyymmdd')) end_date,decode(channel,null,'0',channel) channel,mail,tune,networkid,case when product_name is not null then product_name else hdd_id end product_name,to_char(sysdate+1,'yyyymmdd') ppvdate from ca_queue where sid='178130292'"
        ...
        while 1:
            try:
                ...
                if nop_flag>0:
                    nop_flag = 0
                if sock is None:
                    sock = gw_open("10.21.128.11", 60002)
                    if sock is None:
                        ...
                        for a_row in rs:
                            try:
                                ...
                                if cmd=='A1':
                                    ...
                                    resp = cafunc.gw_reboot_interactive(sid, bg_date, end_date, icc_no)
                                elif cmd=='V10':
