...
class CA_FUNC:
    ...
    def gw_combo(self, seq, sid, bgd, endd, icc, cmd, broadcast='U'):
        sidstr = "%.4d" % (self.sourceid)
        today = time.strftime("%Y%m%d", time.gmtime())
        self.seqid = self.seqid+1
        xseq = "%.9d" % (self.seqid)
        if broadcast=='G':
            str = xseq+'01'+sidstr+self.destid+'44289'+today+'N'+today+endd+broadcast+cmd
        else:
            str = xseq+'01'+sidstr+self.destid+'44289'+today+'N'+today+endd+broadcast+icc+cmd
        print str
    ...
    def gw_repack_icc(self, sid, bgd, endd, icc, stb):
        ...
        if self.gw_combo('02',sid, bgd, endd, iccs, '0023')<0:
            self.rollback_q(2)
            return -1
        if self.gw_combo('03',sid, bgd, endd, iccs, '005200000000000000')<0:
            self.rollback_q(3)
            return -1
        return 1
    ...
    def gw_cancel_prod_1day(self, sid, bgd, endd, icc, channel):
        iccs = icc[:10]
        if self.gw_combo_profile('00',sid, bgd, endd, iccs, '0006'+channel,'U','2')<0:
            return -1
        return 1
	...
	#def gw_channel_change(self, sid, bgd, endd, icc, flag=0, band='L'):
     #   if band == 'L':
     #   	#低頻區
     #   	ird_str = '5B5B5B312A2C322A2C332A2C342A2C352A2C362A2C372A2C382A2C392A5D5D5D0001009303F5'
      #  else:
      #  	#高頻區
       # 	ird_str = '5B5B5B312A2C322A2C332A2C342A2C352A2C362A2C372A2C382A2C392A5D5D5D0001008E000D'
     ...
     #   cmdstr = "193002%.2d%s" % (xlen, ird_str)
     #   cmd = self.ird_cmd(cmdstr)
    #    cmd20 = self.ird_cmd_20(cmdstr)
      #      hdseq = "%.2d" % (curr_seg)
    ...
    def gw_channel_change(self, sid, bgd, endd, icc, profile='N'):
        iccs = icc[:10]
        
        cmd = self.ird_cmd('193002385B5B5B312A2C322A2C332A2C342A2C352A2C362A2C372A2C382A2C392A5D5D5D0001009303F5');
        print 'cmddddd:',cmd
        if self.gw_combo_profile('00', sid, bgd, endd, iccs, cmd, 'U', profile)<0:
            return -1
        return 1    
    
    def gw_allchannel_change(self, sid, bgd, endd, icc, profile='N'):
        iccs = icc[:10]
        
        cmd = self.ird_cmd('193002385B5B5B312A2C322A2C332A2C342A2C352A2C362A2C372A2C382A2C392A5D5D5D0001009303F5');
        print 'cmddddd:',cmd
        if self.gw_combo_profile('00', sid, bgd, endd, iccs, cmd, 'G', profile)<0:
            return -1
        return 1    
        
    def gw_forcetune_music(self, sid, bgd, endd, icc):
        iccs = icc[:10]
        #if tune is None:
        #    return -1
    ...
    def gw_reboot_interactive(self, sid, bgd, endd, icc,nwid):
        iccs = icc[:10]
        cmd = self.ird_cmd('21900100');
        if self.gw_combo_profile('00',sid, bgd, endd, iccs, cmd, 'U', nwid)<0:
            return -1
        return 1
    
    def gw_reboot_interactive2(self, sid, bgd, endd, icc):
        iccs = icc[:10]
        cmd = self.ird_cmd('21900100');
        if self.gw_combo_profile('00',sid, bgd, endd, iccs, cmd, 'U', '3')<0:
            return -1
        return 1
    ...
    def gw_setiad(self, sid, bgd, endd, icc, switch):
        iccs = icc[:10]
        if str(switch) =='1':
          cmdstr = '005'
        else:
          cmdstr = '004'
        cmd = self.ird_cmd('197'+cmdstr+'00');
        if self.gw_combo_profile('00',sid, bgd, endd, iccs, cmd, 'U', '3')<0:
            return -1
        return 1
        
    def gw_stb_cross_region(self, sid, bgd, endd, icc, pid):
        iccs = icc[:10]
        pid  =  hex(int(pid))[2:]  #10進位轉16進位
        #print 'pid:',pid
        #pid = '18B1'
        #zipcode =  binascii.b2a_hex(str(zipcode)) #ascii 轉16進位
        #nwid =  binascii.b2a_hex(str(pid)) #ascii 轉16進位
        #model = hex(int(str(model),2))[2:]
        #cmdstr = nwid + zipcode + model + pid
        cmdstr = '197006105B2A5D5B2A5DFFFF' + str(pid).upper()
        cmd = self.ird_cmd(cmdstr);
