...
querySql = "select icc_no,min(sid) as minsid,max(sid) as maxsid,count(sid) as cnt from ca_queue where sid>260000000 and sourceid is null group by icc_no"
                        ...
                        for a_row in rs:
                            try:
                                ...
                                try:
                                    ...
                                    sourceID_idx = 0
                                    p_sourceID = sourceID_array[sourceID_idx]
                                upd_sql = "update ca_queue set sourceid=%d where icc_no='%s' and sid between %d and %d" % (p_sourceID, icc_no, minsid, maxsid)
