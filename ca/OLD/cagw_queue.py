...
sosql = " and so is null and cmd='E3'"
sourceid = 12

querySql = "SELECT ltrim(to_char(sid,'0999999')) sid,icc_no,stb_no,cmd,decode(bg_date,null,to_char(sysdate,'yyyymmdd'),to_char(bg_date,'yyyymmdd')) bg_date,decode(end_date,null,to_char(sysdate+900,'yyyymmdd'),to_char(end_date,'yyyymmdd')) end_date,decode(channel,null,'0',channel) channel,mail from cagw_queue_d3 where icc_no is not null and status is null %s order by substr(cmd,1,1),sid" % (sosql)
updateSql = "UPDATE cagw_queue_d3 SET status = :1, result=:2, update_date=sysdate,ca_cmd=:3,ca_ret=:4 where sid=:5"
...
def gw_combo(seq,sid, bgd, endd, icc, cmd):
    global sourceid
    sidstr = "%.4d" % (sourceid)
    str = seq+sid+'01'+sidstr+'025744289'+bgd+'N'+bgd+endd+'U'+icc+cmd
    return str 
...
def gw_chgpwd(tn, sid, bgd, endd, icc):
  iccs = icc[:10]
  cmd = ird_cmd('200001050430303030');
...
def gw_mail(tn, sid, bgd, endd, icc, mmsg):
    ...
    while curr_seg<seg:
        ...
        cmd = ird_cmd(cmdstr)
        hdseq = "%.2d" % (curr_seg)
        #print "%s,%d : %s" % (hdseq,xca_mailid,cmd)
        s = gw_combo(hdseq, sid, bgd, endd, iccs, cmd)
        rets =  gw_sendcmd(tn, s)
        print rets
        curr_seg = curr_seg+1
    return rets
     ...
     while 1:
           ...
           if oracon.cexist():
             ...
             if rs != None and len(rs) > 0:
               print rs
               sock = gw_open("10.20.111.11",59001)
               print sock
               if login(sock)>0:
