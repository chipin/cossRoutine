...
querySql = "SELECT ltrim(to_char(sid,'0999999')) sid,icc_no,stb_no,cmd,decode(bg_date,null,to_char(sysdate-(1/3),'yyyymmdd'), \
            ...
            from %s where icc_no is not null and status is null %s and (active_time is null or sysdate>=active_time) order by substr(cmd,1,1),sid" % (q_table, sosql)
