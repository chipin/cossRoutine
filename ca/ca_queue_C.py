...
querySql = "select ltrim(to_char(sid)) sid,icc_no,stb_no,cmd,decode(bg_date,null,to_char(sysdate-(1/3),'yyyymmdd'),to_char(bg_date-1,'yyyymmdd')) bg_date,decode(end_date,null,to_char(sysdate+3650,'yyyymmdd'),to_char(end_date,'yyyymmdd')) end_date,decode(channel,null,'0',channel) channel,mail,tune,networkid,product_name,to_char(sysdate+1,'yyyymmdd') ppvdateso,src,decode(account,'PRE_ACTIVE',9,0) orderby from ca_queue where cmd ='A6' and sid='327433996'  "
print querySql
xca_cmd = ''
xca_ret = ''
...
def ora_upd(ora, sid, status, result=None, ca_cmd=None, ca_ret=None):
    try:
        ...
        sql = "update ca_queue set status='%s'%s,update_date=sysdate where sid=%d" % (status, result_sql, int(sid))
        #print sql
        ora.execone(sql)
        return 1
    except Exception, msg:
                        ...
                        for a_row in rs:
                            try:
                                ...
                                if cmd=='A1':
                                    ...
                                    resp = cafunc.gw_add_prod_7days(sid, bg_date, end_date, icc_no, channel, '1')
                                elif cmd=='B8':
                                    ...
                                    resp = cafunc.gw_add_prod_62days(sid, bg_date, end_date, icc_no, channel, '1')
                                elif cmd=='B20P':
                                	...
                                	resp = cafunc.gw_channel_change(sid, bg_date, end_date, icc_no, 0, 'L')
                                elif cmd=='E4':
                                    resp = cafunc.gw_forcetune(sid, bg_date, end_date, icc_no)
                                elif cmd=='E4U':
                                    ...
                                    resp = cafunc.gw_networkid_by_profile(sid, bg_date, end_date, icc_no, networkid, '1')
                                elif cmd=='E6':
