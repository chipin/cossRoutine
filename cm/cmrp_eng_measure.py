...
## 9 CMC
if so == 'TFM':
    querysql = "select a.companyno,a.subsid,a.worksheet,a.singlesn,a.worker1,convert(varchar(19),a.finishtime,20) finishtime,b.workkind from ms0301 a with (nolock), ms0300 b with (nolock) \
                where a.companyno=b.companyno and a.worksheet=b.worksheet and substring(b.workkind,1,1) in ('1','2','6','9','C','5') and substring(a.sheetstatus,1,1) not in ('3','A') \
                and a.companyno in ('101','103','104','300','701') and a.servicename='9 CMC' and a.singlesn is not null and a.singlesn<>'' and convert(varchar,a.opfintime,112) ='%s' \
                group by a.companyno,a.subsid,a.worksheet,a.singlesn,a.worker1,a.finishtime,b.workkind" % (yyyymmdd)
else:
    querysql = "select a.companyno,a.subsid,a.worksheet,a.singlesn,a.worker1,convert(varchar(19),a.finishtime,20) finishtime,b.workkind from ms0301 a with (nolock), ms0300 b with (nolock) \
                where a.companyno=b.companyno and a.worksheet=b.worksheet and substring(b.workkind,1,1) in ('1','2','6','9','C','5') and substring(a.sheetstatus,1,1) not in ('3','A') \
                and a.servicename='9 CMC' and a.singlesn is not null and a.singlesn<>'' and convert(varchar,a.opfintime,112) ='%s' \
                group by a.companyno,a.subsid,a.worksheet,a.singlesn,a.worker1,a.finishtime,b.workkind" % (yyyymmdd)
print querysql
...
while 1:
    ...
    if curarr:
        ...
        for ii in range(0, xlen):
            ...
            mac       = curarr[ii][3]
            worker    = curarr[ii][4]
            finishtime= curarr[ii][5]
            workkind  = curarr[ii][6]
            work_id   = ''
            work_name = ''
            try:
            ...
            qry_sql = "select count(*) cnt from eng_tickets_main where companyno='%s' and subsid='%d' and worksheet='%s' and mac='%s'" % (companyno,subsid,worksheet,mac)
            ...
            if cnt < 1:
                if cmussnr is None or cmdssnr is None:
                    oraupdsql = "insert into eng_tickets_main(companyno,worksheet,servicename,subsid,work_id,worker,mac,memo,eng_datetime, workkind, updatetime) \
                                 values ('%s','%s','9 CMC',%d,'%s','%s','%s','9CMC',sysdate,'%s',sysdate)" % (companyno, worksheet, subsid, work_id, work_name, mac, workkind)
                else:
                    oraupdsql = "insert into eng_tickets_main(companyno,worksheet,servicename,subsid,work_id,worker,mac,memo,eng_datetime, cmussnr,cmussnr_2, cmdssnr, cmuspwr, cmuspwr_2, cmdspwr,uprxpwr, uprxpwr_2, t3, t4, workkind, updatetime) \
                                 values('%s','%s','9 CMC',%d,'%s','%s','%s','9CMC',sysdate,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',sysdate)" % (companyno, worksheet, subsid, work_id, work_name, mac, cmussnr, cmussnr_2, cmdssnr, cmuspwr, cmuspwr_2, cmdspwr, uprxpwr, uprxpwr_2, t3, t4, workkind)
            else:
                if cmussnr is None or cmdssnr is None:
                    ...
                    oraupdsql = "update eng_tickets_main set cmussnr='%s',cmussnr_2='%s',cmdssnr='%s',cmuspwr='%s',cmuspwr_2='%s',cmdspwr='%s',uprxpwr='%s',uprxpwr_2='%s',servicename='9 CMC',work_id='%s',worker='%s',workkind='%s',updatetime=sysdate where companyno='%s' and subsid='%d' and worksheet='%s' and mac='%s'" %  \
...
if so == 'TFM':
   ...
   querysql = "select a.companyno,a.subsid,a.worksheet,a.singlesn,c.stbcmmac,a.worker1,convert(varchar(19),a.finishtime,20) finishtime,b.workkind \
               ...
               and convert(varchar,a.finishtime,112) ='%s' group by a.companyno,a.subsid,a.worksheet,c.stbcmmac,a.worker1,a.finishtime,b.workkind,a.singlesn" % (yyyymmdd)
	    ...
	    if so == 'TFM':
	       ...
	       qry_sql = "select rxpwr,cm_rxpwr,cm_txpwr,dl_snr,ul_snr,cm_t3,cm_t4,ul_snr2,rxpwr2,cm_txpwr2 from cmmac where cmmac='%s' and companyno='%s'" % (mac, companyno)
               ...
               if cnt < 1:
                 if cmussnr is None or cmdssnr is None:
                    oraupdsql = "insert into eng_tickets_main(companyno,worksheet,servicename,subsid,work_id,worker,mac,memo,eng_datetime, workkind, updatetime) \
                                 values ('%s','%s','3 DSTB',%d,'%s','%s','%s','3DSTB',sysdate,'%s',sysdate)" % (companyno, worksheet, subsid, work_id, work_name, mac, workkind)
                 else:
                    oraupdsql = "insert into eng_tickets_main(companyno,worksheet,servicename,subsid,work_id,worker,mac,memo,eng_datetime, cmussnr,cmussnr_2, cmdssnr, cmuspwr, cmuspwr_2, cmdspwr,uprxpwr, uprxpwr_2, t3, t4, workkind, updatetime) \
                                 values('%s','%s','3 DSTB',%d,'%s','%s','%s','3DSTB',sysdate,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',sysdate)" % (companyno, worksheet, subsid, work_id, work_name, mac, cmussnr, cmussnr_2, cmdssnr, cmuspwr, cmuspwr_2, cmdspwr, uprxpwr, uprxpwr_2, t3, t4, workkind)
               else:
                 if cmussnr is None or cmdssnr is None:
                    oraupdsql = "update eng_tickets_main set work_id='%s',worker='%s',workkind='%s',updatetime=sysdate where companyno='%s' and subsid='%d' and worksheet='%s' and mac='%s'" % \
                                 (work_id, work_name, workkind, companyno, subsid, worksheet, mac)
