...
def login_mml(tn):
  ...
  tn.write('lgi:op="tfmcnisprov",PWD="6&yUhJnM";\n')
...
if login_ok == 1:
    mml = 'LST VSBR: EID="'+mac+'.cable.tfn.net.tw",TID="'+tid+'",SCF=NO,SRF=YES,SOF=NO,SGF=NO;'
    print mml
    resp = sendMML(mmtln,mml)
    ...
    for idx in lnidx:
        ...
        if i<7:
            continue
        match_token(idx.lstrip())

    #mml = 'DSP EPST: TRMTYPE=TRM1, EID="'+mac+'.cable.tfn.net.tw",TRMID='+tid+',QUERYBY=1;'
    mml = 'DSP MGW: EID="'+mac+'.cable.tfn.net.tw";'
    ...
    idx = len(lnidx)-6
    print "EPST=",
    if lnidx[idx].find("Module")>=0:
        tkn = lnidx[idx+2].split()
        for opt in tkn:
            print opt+',',
    print ""
else:
