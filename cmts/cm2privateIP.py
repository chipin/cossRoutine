...
def getIP(so,cmmac):
    ...
    if  not ora_nms.db:
        ...
        return {'rtn':'err','msg':'[' + nowdate + ']: Unable to connect to server [CNIS]'}
    # ------------- Get CMTS infomation -------------------------------------------
    SQL = "select cmts_id,case when companyno='101' and ip2 is not null then ip2 else ip end ip,snmp_ro from cmts where companyno='%s' and stopyn='N' order by cmts_id" % (so)
    rst = ora_nms.execall(SQL)
    if  rst is None:
        ora_nms.se_close()
        return {'rtn':'err','msg':'Error: Unable to query CMTS [CNIS]'}
    else:
        if len(rst)>0:
            i = 0
            for aw in rst:
            ...
            return {'rtn':'err','msg':'Error: Unable to query CMTS [CNIS]'}
    # ------------- Get measure items -------------------------------------------
    SQL = "select oid, ip_type, v_type, operator from cm_measure_item where id in ('86','101','102','113','1','2','3','87','94','95','114','118','117','119') order by id"
    rst = ora_nms.execall(SQL)
    if  rst is None:
        ora_nms.se_close()
        return {'rtn':'err','msg':'Error: Unable to query cm_measure_item [CNIS]'}
    else:
        if  len(rst)>0:
            cmtsidx = 0
            mitem['COUNT'] = 0
    ...
    if  valid_cmts_idx>=0:
        ...
        i = 0
        while i < mitem['COUNT']:
            if  mitem[str(valid_cmts_idx)+'-V-'+str(i)] is not None:
                # 查詢cmmac 取得 privateIP
                if  i==1:
                    cmmac2ip.append(mitem[str(valid_cmts_idx)+'-V-'+str(i)])
                    break;
            i = i+1
        return {'rtn':'ok','msg':cmmac2ip}
    else:
        return {'rtn':'err','msg':'data none'}