...
someday = time.localtime(time.time()-10*24*60*60)
...
if so in ['101','103','104','300','701']:
    querysql = "select a.companyno,case when a.nodeno is not null and a.nodeno <> '' and a.nodeno <> '未設' then a.nodeno else 'ZZ999' end nodeno, \
                sum(case when substring(b.servicename,1,1) = '1' then 1 else 0 end) catv_cnt,sum(case when substring(b.servicename,1,1) in ('2','9') or (f.stb_cm_mac is not null and f.stb_cm_mac <> '') then 1 else 0 end) cm_cnt, \
                sum(case when substring(b.servicename,1,1) = '3' then 1 else 0 end) dtv_cnt,sum(case when substring(b.servicename,1,1) = '5' then 1 else 0 end) fttb_cnt,sum(case when substring(b.servicename,1,1) = '7' then 1 else 0 end) eoc_cnt,sum(case when substring(b.servicename,1,1) = '8' then 1 else 0 end) ll_cnt, \
                c.mscity+c.msdistrict+c.locateaddr addr,count(distinct(a.linkid)) link_cnt \
                from ms0102 a with (nolock) \
                inner join ms0200 b with (nolock) on b.companyno=a.companyno and b.custid=a.custid and substring(b.custstatus,1,1) in ('0','1','2','6','7','8','9','A','B') \
                inner join ms0020 c with (nolock) on c.companyno=a.companyno and c.netid=a.nodeno and c.mslevel='20' \
                left join nwis0020 f with (nolock) on (f.companyno=b.companyno or f.companyno is null) and f.singlesn=b.singlesn and f.stb_cm_mac is not null and f.stb_cm_mac <> '' and f.mtkind='STB' and substring(b.servicename,1,1) = '3' and b.singlesn is not null and b.singlesn <> '' \
                where a.companyno='%s' and a.addrno='0' \
                group by a.companyno,a.nodeno,c.mscity,c.msdistrict,c.locateaddr order by a.companyno,a.nodeno" % (so)
else:
    querysql = "select a.companyno,case when a.nodeno is not null and a.nodeno <> '' and a.nodeno <> '未設' then a.nodeno else 'ZZ999' end nodeno, \
                sum(case when substring(b.servicename,1,1) = '1' then 1 else 0 end) catv_cnt,sum(case when substring(b.servicename,1,1) in ('2','9') or (f.stbcmmac is not null and f.stbcmmac <> '') then 1 else 0 end) cm_cnt, \
                sum(case when substring(b.servicename,1,1) = '3' then 1 else 0 end) dtv_cnt,sum(case when substring(b.servicename,1,1) = '5' then 1 else 0 end) fttb_cnt,sum(case when substring(b.servicename,1,1) = '7' then 1 else 0 end) eoc_cnt,sum(case when substring(b.servicename,1,1) = '8' then 1 else 0 end) ll_cnt, \
                c.mscity+c.msdistrict+c.locateaddr addr,count(distinct(a.linkid)) link_cnt \
                from ms0102 a with (nolock) \
                inner join ms0200 b with (nolock) on b.companyno=a.companyno and b.custid=a.custid and substring(b.custstatus,1,1) in ('0','1','2','6','7','8','9','A','B') \
                inner join ms0020 c with (nolock) on c.companyno=a.companyno and c.netid=a.nodeno and c.mslevel='20' \
                left join mi0130 f with (nolock) on f.companyno=b.companyno and f.singlesn=b.singlesn and f.stbcmmac is not null and f.stbcmmac <> '' and substring(b.servicename,1,1) = '3' and b.singlesn is not null and b.singlesn <> '' \
                where a.companyno='%s' and a.addrno='0' \
                group by a.companyno,a.nodeno,c.mscity,c.msdistrict,c.locateaddr order by a.companyno,a.nodeno" % (so)
tme = time.strftime("%Y/%m/%d %H:%M:%S", time.localtime())
print "[%s]" % (tme)
...
while 1:
    ...
    if curarr:
        ...
        for ii in range(0, xlen):
            ...
            cm_cnt = curarr[ii][3]
            dtv_cnt = curarr[ii][4]
            fttb_cnt = curarr[ii][5]
            eoc_cnt = curarr[ii][6]
            ll_cnt = curarr[ii][7]
            addr = curarr[ii][8]
            link_cnt = curarr[ii][9]
            print companyno,nodeno,addr,link_cnt
            oraupdsql = "begin proc_cnis_upd_node_info('%s','%s','%d','%d','%s','%d','%d','%d','%d','%d'); end;" % (companyno,nodeno,catv_cnt,cm_cnt,addr,dtv_cnt,fttb_cnt,eoc_cnt,ll_cnt,link_cnt)
            print oraupdsql
            try:
...
if mode == 'full':
    querysql = "select b.companyno,b.subsid,b.singlesn cmmac,b.custstatus,a.mscity,a.msdistrict,a.msregion,a.msroad,a.addrname, \
                ...
                from ms0102 a with (nolock) \
                inner join ms0200 b with (nolock) on b.companyno=a.companyno and b.custid=a.custid and substring(b.custstatus,1,1) in ('0','1','2','6','7','8','9','A','B') and substring(b.servicename,1,1) = '9' and b.singlesn is not null and b.singlesn <> '' \
...
if mode == 'full':
    querysql = "select b.companyno,b.subsid,b.singlesn cmmac,b.custstatus,a.mscity,a.msdistrict,a.msregion,a.msroad,a.addrname, \
                ...
                where a.companyno='%s' and a.addrno='0'" % (so,so,so)
else:
    querysql = "select b.companyno,b.subsid,b.singlesn cmmac,b.custstatus,a.mscity,a.msdistrict,a.msregion,a.msroad,a.addrname, \
                case when a.nodeno is not null and a.nodeno <> '' and a.nodeno <> '未設' then a.nodeno else 'ZZ999' end nodeno, \
                case when a.linkid is not null and a.linkid <> '' and a.linkid <> '未設' then a.linkid else 'ZZ999-999999' end linkid, \
...
if so in ['101','103','104','300','701']:
    if mode == 'full':
        querysql = "select b.companyno,b.subsid,f.stb_cm_mac cmmac,b.custstatus,a.mscity,a.msdistrict,a.msregion,a.msroad,a.addrname, \
                    case when a.nodeno is not null and a.nodeno <> '' and a.nodeno <> '未設' then a.nodeno else 'ZZ999' end nodeno, \
                    case when a.linkid is not null and a.linkid <> '' and a.linkid <> '未設' then a.linkid else 'ZZ999-999999' end linkid, \
                    b.packagename,b.billitem,b.swversion,b.swversion2,b.singlesn,b.smartcard,b.chargename,b.singlesn2,b.chargename2, \
                    ...
                    inner join nwis0020 f with (nolock) on (f.companyno=b.companyno or f.companyno is null) and f.singlesn=b.singlesn and f.stb_cm_mac is not null and f.stb_cm_mac <> '' and f.mtkind='STB' \
