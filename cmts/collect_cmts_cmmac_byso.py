...
while cmts_idx<cmts_cnt:
    try:
        ...
        SQL = "update cmts set mflag=-1,synctime=sysdate,syncmactime=sysdate where companyno='%s' and cmts_id='%s'" % (xso, xcmtsid)
        ora.execone(SQL)
        ora.commit()
        cmts_idx = cmts_idx+1
        continue
    ...
    SQL = "update cmts set mflag=0,sys_descr='%s',synctime=sysdate,syncmactime=sysdate where companyno='%s' and cmts_id='%s'" % (cmts_descr, xso, xcmtsid)
    ora.execone(SQL)
    ora.commit()
...
if so.upper() == 'TFM1' :
    sql = 'delete from cmts_if where synctime < sysdate-8 or ifdescr is null'
    print sql
    ora.execone(sql)
    ora.commit()

    sql = "update cmmac_2min set closeuser='SYS',closetime=sysdate,closeremark='《監控天數到期,系統自動關閉》' where (closetime is null or closetime < opentime) and opentime < sysdate-14"
    print sql
    ora.execone(sql)
    ora.commit()
