...
while 1:
    ...
    if exec_time-pre_exec_time>300:
        ...
        SQL = "select name,upper(model) model,oid \
               from snmp_mib \
               where name in ('cmtsifDSCmActive','cmtsifDSCmTotal','cmtsifUSCmActive','cmtsifUSCmTotal')"
        rst = ora.execall(SQL)
        ...
        cmtssql = "select a.companyno,a.cmts_id,a.ip,a.snmp_ro, \
		     ...
		     when upper(b.maker||b.model) like '%%CASA%%' then 'CASA' \
                   ...
                   where " + sosql + " and a.ip is not NULL and a.snmp_ro is not NULL and a.stopyn='N' \
        ...
        while cmts_idx<cmts_cnt:
            try:
                ...
                if residx is not None and len(residx)>0:
                    ...
                    for xid in residx:
                        try:
                            cmactive = int(result[xcmtsid+'-cmtsifDSCmActive-'+str(xid)])
                        except:
                            try:
                                cmactive = int(result[xcmtsid+'-cmtsifUSCmActive-'+str(xid)])
                                t_cmactive = t_cmactive + cmactive
                            except:
                                cmactive = -1
                        try:
                            cmtotal = int(result[xcmtsid+'-cmtsifDSCmTotal-'+str(xid)])
                        except:
                            try:
                                cmtotal = int(result[xcmtsid+'-cmtsifUSCmTotal-'+str(xid)])
