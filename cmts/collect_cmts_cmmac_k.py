...
if so.upper() == 'TFM1':
    ...
    sql = "update cmmac_2min set closeuser='SYS',closetime=sysdate,closeremark=closeremark||'《監控天數到期,系統自動關閉》' where (closetime is null or closetime < opentime) and opentime < sysdate-7"
    print sql
    ora.execone(sql)
    ora.commit()
