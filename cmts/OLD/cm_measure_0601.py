...
def snmp_walk(ip, oid, types, comm):
    ...
    try:
	print oid
        agt = snmpclass(version='v2c', community=comm, ptimeout=1, pretries=3, debug=0)
...
# ------------- Get CMTS infomation -------------------------------------------
SQL = "select cmts_id,ip,snmp_ro from cmts where companyno='%s' order by cmts_id" % (so)
...
# ------------- Get measure items -------------------------------------------
SQL = "select oid, ip_type, v_type, operator from cm_measure_item where id in ('78','98','27','96','18','97','4','19','90','91') order by id"
