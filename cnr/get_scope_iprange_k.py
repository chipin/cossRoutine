...
def upd2oradb(companyno, scopename, cnr_host, addr, hosts, cidr, ipnum_start, ipnum_end, cnr_id, oracon) :
    try:
        ins_cnr_scope_iprange = "insert into cnr_scope_iprange_test (companyno, scope, cnrip, subnet, hosts, prefix, ip_bgv, ip_endv, cnr_id, updtime, lastest) values ('" + str(companyno) + "','" + scopename + "','" + cnr_host + "','" + addr + "','" + str(hosts) + "','" + str(cidr) + "','" + str(ipnum_start) + "','" + str(ipnum_end) + "','" + cnr_id + "',sysdate,'1')"
        print ins_cnr_scope_iprange
        oracon.execone(ins_cnr_scope_iprange)
    except Exception, detail:
        upd_cnr_scope_iprange = "update cnr_scope_iprange_test set companyno='" + str(companyno) + "', scope='" + scopename + "', cnrip='" + cnr_host + "', subnet='" + addr + "', hosts='" + str(hosts) + "', prefix='" + str(cidr) + "', ip_bgv='" + str(ipnum_start) + "', ip_endv='" + str(ipnum_end) + "', cnr_id='" + cnr_id + "', updtime=sysdate, lastest='1' where scope='" + scopename + "'"
...
qrysql = "select ip,apiuser,apipwd,companyno,cnr_id from cnr where stopyn='N' and ip is not null"
so = sys.argv[1].upper()
if so == 'ALL':
...
for row_cnr_info in rs_cnr_info:
    try:
        ...
        if companyno in ['810','820']:
            updsql = "update cnr_scope_iprange_test set lastest='0',cmts_id=null,bundle_name=null where lastest != '2' and companyno in ('810','820')"
        else:
            updsql = "update cnr_scope_iprange_test set lastest='0',cmts_id=null,bundle_name=null where lastest != '2' and cnr_id='" + cnr_id + "' and companyno='" + companyno + "'"
        print updsql
        try:
          oracnis.execone(updsql)
          oracnis.commit()
        except Exception, detail:
          print 'Exception: update lastest: %s' % (detail)
        ...
        if companyno in ['810','820']:
            updsql = "delete from cnr_scope_iprange_test where lastest='0' and companyno in ('810','820')"
        else:
            updsql = "delete from cnr_scope_iprange_test where lastest='0' and cnr_id='" + cnr_id + "' and companyno='" + companyno + "'"
        print updsql
        try:
          oracnis.execone(updsql)
          oracnis.commit()
        except Exception, detail:
          print 'Exception: delete lastest: %s' % (detail)
        ...
        for row1 in rs1:
            ...
            for bb in scope_arr:
                if companyno in ['810','820']:
                    updsql2 = "update cnr_scope_iprange_test set cmts_id='%s',bundle_name='%d' where companyno in ('810','820') and scope like '%s%%'" % (cmts_id, bundle, bb)
                else:
                    updsql2 = "update cnr_scope_iprange_test set cmts_id='%s',bundle_name='%d' where companyno='%s' and scope like '%s%%'" % (cmts_id, bundle, companyno, bb)
                print updsql2
                try:
                  oracnis.execone(updsql2)
                  oracnis.commit()
                except Exception, detail:
                  print 'Exception: Update cmts: %s' % (detail)
                bundle = bundle + 1
