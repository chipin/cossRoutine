...
def query_status(yneid, yip, ymodel, ycomm, yoid):
    global snmp_oid
    print yoid
    print yneid, yip, ymodel, ycomm

    if ymodel.find('DEV') >= 0:
...
qrysql = "select a.companyno,a.dtv_id,a.ip,a.snmp_ro,a.mflag,b.maker||' '||b.model model \
          from dtv_ne a \
          inner join sys_object b on b.ne_id=substr(a.dtv_id,-8,4) and b.stopyn='N' \
          where " + sosql + " and a.ip is not NULL and a.snmp_ro is not NULL and a.stopyn='N' and a.mflag > 0 \
          order by a.companyno,a.dtv_id asc"
#print qrysql
...
while ne_idx<ne_cnt:
    try:
        ...
        #result = {}
        #result = query_status(xneid, xIP, xmodel, xcomm, oid_name)
        ##print 'result=',result
        #
        #ifnum = 0
        #try:
        ...
        #        SQL = "update ip_ne_if set ifdescr='%s',ifadminstatus='%s',ifoperstatus='%s',ifspeed=%d,ifalias='%s',synctime=sysdate where companyno='%s' and ne_id='%s' and ifindex=%d" % (result[xneid+'-ifDescr-'+str(xid)], result[xneid+'-ifAdminStatus-'+str(xid)], result[xneid+'-ifOperStatus-'+str(xid)], int(result[xneid+'-ifSpeed-'+str(xid)]), xifalias, xso, xneid, int(xid))
        #    else:
        #        SQL = "insert into ip_ne_if (companyno,ne_id,ifindex,ifdescr,ifspeed,ifadminstatus,ifoperstatus,ifalias,synctime) values ('%s','%s',%d,'%s',%d,'%s','%s','%s',sysdate)" % (xso, xneid, int(xid), result[xneid+'-ifDescr-'+str(xid)], int(result[xneid+'-ifSpeed-'+str(xid)]), result[xneid+'-ifAdminStatus-'+str(xid)], result[xneid+'-ifOperStatus-'+str(xid)], xifalias)
        #    print i, '/', ifnum, ':', SQL
        #    ora.execone(SQL)
        #    i = i+1
        #    if (i%50)==0:
        ...
        SQL = "update dtv_ne set mflag=-1,synctime=sysdate where companyno='%s' and dtv_id='%s'" % (xso, xneid)
        ora.execone(SQL)
        ora.commit()
        ne_idx = ne_idx+1
        continue
    ...
    SQL = "update dtv_ne set mflag=%d,sys_descr='%s',synctime=sysdate where companyno='%s' and dtv_id='%s'" % (mflag, sys_descr, xso, xneid)
    ora.execone(SQL)
    ora.commit()
