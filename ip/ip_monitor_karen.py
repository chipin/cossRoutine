...
# -*- coding: utf-8 -*-
# 中文字編碼
import sys, re, time, threading,os
import MySQLdb
import locale
...
def IP_Alarm(oemsno,companyno,ne_type,ne_id,ifindex,alarm_type,alarm_status,alarm_msg):
    global oraoems, oraqos,oems_sid,access_aid,access_type,access_tickets

    
    
    print access_type,':',oemsno,':',companyno,':',ne_type,':',ne_id,':',ifindex,':',alarm_type,':',alarm_status,':',alarm_msg
    ...
    if alarm_type in ['IN_MAX','OUT_MAX']:
        normal_flag = 'I'
        if access_type == 'Transit 電路':
              type = 4308
        elif access_type == 'ISC 電路':
              type = 4309
        elif access_type == 'Hub backhaul 電路':
              type = 4310
        elif access_type == 'CMTS GE 電路':
              type = 4311
        elif access_type == 'FTTx GE 電路':
              type = 4312
        elif access_type == 'CDN 電路':
              type = 4313
        elif access_type == 'Server 電路':
              type = 4314
        elif access_type == 'DCN 電路':
              type = 4315
    ...
    try:
        if normal_flag == 'I':
                if access_sid[ne_id + '-' + str(ifindex) + '-' + alarm_type] is not None:
                        access_id = access_sid[ne_id + '-' + str(ifindex) + '-' + alarm_type]
                        
                        if access_sid[ne_id + '-' + str(ifindex) + '-' + alarm_type + 'status'] is not None:
                            access_status = access_sid[ne_id + '-' + str(ifindex) + '-' + alarm_type + 'status']

                        if access_sid[ne_id + '-' + str(ifindex) + '-' + alarm_type + 'statustime'] is not None:
                            statustime = access_sid[ne_id + '-' + str(ifindex) + '-' + alarm_type + 'statustime'] 
    ...
    if alarm_status == 1:
        ...
        if normal_flag == 'I':
                if access_id > 0: # 持續異常
                    print 'id:',access_id,'-statustime:',statustime,'-status:',access_status
                    action_flag = 'LOG'
        ...
        if normal_flag == 'I':
                if access_id > 0: # 恢復正常
                    ...
                    if access_status == 5104:
                        uptsql = "update access_tickets_main set status = 5105, closetime=sysdate where aid = '%d'" %(access_id)
                        print uptsql
                        try:
                            oraqos.execone(updsql)
                            #pass
                        except:
                            print 'Exception: Unable to update Access_TICKETS_LOG'
                ...
                if oems_id > 0: # 恢復正常
                    ...
                    pass
    print 'action_flag:',action_flag
    if action_flag == 'NEW':
        if normal_flag == 'I':
            aid = 0
            updsql = "begin insert into access_tickets_main (companyno,type,status,reason,descr,cmts_id,ifindex,remarks,createuser,createtime) values (%d,%d,%d,'%s','%s','%s','%d','%s','IPMONv2',sysdate) return to_char(aid) into :1 ;end;"  %(int(companyno),type,status,alarm_reaon,alarm_msg,ne_id,int(ifindex),alarm_type)
            print updsql
            try:
                access_aid_ary = oraqos.db.BindingArray(1,12,'SQLT_STR')
                oraqos.c.execute(updsql, access_aid_ary);
                aid = int(access_aid_ary[0])
                print 'aid:',aid
            except:
                print 'Exception: Unable to insert Access_TICKETS_MAIN (NEW)'
            
            if aid > 0:
                updsql = "insert into access_tickets_log (aid,createtime,orig_status,status,descr,createuser) values (%d,sysdate,%d,%d,'%s','IPMONv2')" % (aid,status,status,alarm_msg)
                print updsql
                try:
                    oraqos.execone(updsql)
                    #pass
                except:
                    print 'Exception: Unable to insert Access_TICKETS_LOG (NEW)'
            else:
                print 'Exception: Unable to insert Access_TICKETS_LOG (NEW)'
        ...
        if normal_flag == 'I':
            if access_id > 0:
                updsql = "insert into access_tickets_log (aid,createtime,orig_status,status,descr,createuser) values (%d,sysdate,%d,%d,'%s','IPMONv2')" % (access_id,access_status,access_status,alarm_msg)
                print updsql
                try:
                    oraqos.execone(updsql)
                    #pass
                except:
                    print 'Exception: Unable to insert Access_TICKETS_LOG (LOG)'
            else:
                print 'Exception: No Access_id, not insert Access_TICKETS_LOG (LOG)'
        else:
            if oems_id > 0:
...
where a.account='IPMONv2' and a.close_date is null and a.normal_flag in ('A','B') and a.type in ('3111','4304') and a.status not in ('5029','5125') and a.impact_list like '%%-%%' and a.special_flag is not null and a.create_date > to_date('2016/01/01','YYYY/MM/DD') and a.location='%s'" % (so)
    #print qry_sql
    ...
    qry_sql2 = "select a.aid,a.cmts_id,a.ifindex,a.remarks,a.status,a.statustime from access_tickets_main a where a.createuser='IPMONv2' and a.closetime is null and a.status not in ('5109','5105') and companyno ='%s'" %(so)
    print qry_sql2
    rs2 = oraqos.execall(qry_sql2)

    if rs2 is not None and len(rs2) > 0:
        for az in rs2:
            access_sid[az[1] + '-' + str(int(az[2])) + '-' + az[3]] = int(az[0])
            access_sid[az[1] + '-' + str(int(az[2])) + '-' + az[3] + 'status'] = int(az[4])
            access_sid[az[1] + '-' + str(int(az[2])) + '-' + az[3] + 'statustime'] = az[5]
            
    print 'ACCESS_ID:',access_sid
...
a.k_top_in,a.k_max_in,a.k_min_in,a.k_increase_in,a.k_decrease_in,a.k_top_out,a.k_max_out,a.k_min_out,a.k_increase_out,a.k_decrease_out,a.alarm_type \
from ipif_kpi a \
...
order by companyno,ne_id,ifindex" % (so,so)
    ...
    for xx in range(len(ip_ne)):
        ...
        #print '＊',soname,companyno,ne_type,ne_id,ifindex,ifdescr,'IN_TOP:',k_top_in,'IN_MAX:',k_max_in,'IN_MIN:',k_min_in,'IN_INC:',k_increase_in,'IN_DEC:',k_decrease_in,'OUT_TOP:',k_top_out,'OUT_MAX:',k_max_out,'OUT_MIN:',k_min_out,'OUT_INC:',k_increase_out,'OUT_DEC:',k_decrease_out,'alarm_type',alarm_type
        ...
        if k_max_in > 0:
            status = 0
            if inbps1 >= k_max_in and inbps1 > 0 and alarm_type is not None:
                status = 1
        
            #msg = "[告警] - %s 設備編號:%s, 介面名稱:%s, 流量超標, 目前流量:$sbps" % (soname,ne_id,ifdescr,locale.format("%d",inbps1,1))
            access_type = alarm_type 
        ...
        if k_max_out > 0:
            status = 0
            if outbps1 >= k_max_out and outbps1 > 0 and alarm_type is not None:
                status = 1
            access_type = alarm_type
