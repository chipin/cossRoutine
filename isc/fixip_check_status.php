<?php
  ...
  */
  ...
  $nms = GetDBH('CNIS');
  $database_array = array("CG","TFM","KBRO");
  ...
  $sql = "select a.subsid,a.companyno,a.ip,a.chargename,a.pay,a.mac,b.cnr_id from cnr_fixip_coss a  left join cnr_queue b  on  a.cnr_id = b.sid where a.pay is not null and a.stopyn = 'N'  ";
  ...
  //-------查詢cnr_id-----s
  $cnr_ary = array();
  $sql = "select companyno,cnr_id,header,cgnat_flag from cnr where stopyn = 'N'";
  $sth = oci_parse($nms,$sql);
  oci_execute($sth);
  while($row = oci_fetch_assoc($sth)){
    $cnr_ary[$row['COMPANYNO']]['cnr_id'] = $row['CNR_ID'];
    $cnr_ary[$row['COMPANYNO']]['header'] = $row['HEADER'];
    $cnr_ary[$row['COMPANYNO']]['cgnat_flag'] = $row['CGNAT_FLAG'];
  }
  ...
  //確認全部的付費固定IP狀態-----e
  //$result1 = 比對兩個名單, 以震江(1)的名單為主, 沒在名單中的要做以下的事情
  //震江付費固定ip狀態為不可使用, 但cnr_fixip_coss 還可使用, 需移除。
//  echo '<pre>';print_r($result1);
  foreach($result1 as $item){
    ...
    $cnr_id = $cnr_ary[$companyno]['cnr_id'];

    $result1_ms_0210_temp = $result1_ms_0210[$item['COMPANYNO']][$item['SUBSID']][$item['CHARGENAME']];
    if (preg_match("/^(DE|DA)-.*/i", $result1_ms_0210_temp['ori_companyno'])){
      if($result1_ms_0210_temp['disctime']<time()){
        ...
        $command = 'delfixip';
        $check_vip = 0;
        $sql_ins = "insert into cnr_queue(cnr_id,command,ip,cpemac,companyno,subsid,account)values('$cnr_id','$command','$ip','$mac','$companyno','$subsid','cronPayFixIP')";
        ...
        if ($cnr_ary[$companyno]['cgnat_flag'] == 1){
           $sql_ins2 = "insert into cnr_queue(cnr_id,command,ip,cpemac,companyno,subsid,account)values('$cnr_id','fixprivate','','$mac','$companyno','$subsid','cronPayFixIP')";
           $sth_ins2 = oci_parse($nms, $sql_ins2);
           oci_execute($sth_ins2);
           $sql_upt = "update cnr_ip_public set stopyn='Y',updateuser='cronPayFixIP',updatetime=sysdate where companyno='$companyno' and subsid='$subsid' and cpemac='$mac'";
        ...
        $ip_c = strtok('.');
        $ip_d = strtok('.');
        $ip_v = $ip_a*pow(256,3)+$ip_b*pow(256,2)+$ip_c*256+$ip_d;
        $sql_qry = "select * from cnr_scope_iprange where  $ip_v between ip_bgv and ip_endv and scope like '%B70'";
        $qrysth = oci_parse($nms,$sql_qry);
        oci_execute($qrysth);
        if ($qryrow = oci_fetch_assoc($qrysth)){
          $check_vip = 1;
        }
        
        if ($pay == 'lock' && $check_vip == 0){
          $com ='activeip';
          $ins = "insert into cnr_queue(cnr_id,command,ip,companyno,subsid,account) values('$cnr_id','$com','$ip','$companyno','$subsid','cronPayFixIP')";
        }
        echo $command.'/'.implode('/', $item).' -> '.$sql_ins.' -> '.$sql_del. '-> '.$ins . '-> '.$sql_ins2. '-> '.$sql_upt;echo "<br>\n";
  ...
  //$result2 = 比對兩個名單, 以震江(1)的名單為主, 有在名單中的要做以下的事情
  //震江付費固定IP已付費或可使用, 但cnr_fixip_coss目前狀態為不可使用
