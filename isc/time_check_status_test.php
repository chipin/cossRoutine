<?php
  ...
  */
  ...
  if ($argv[1] == 'KBRO1'){
    $sosql = "'210','220','230','240','250','260'";
    $dbh = GetDBH('kbroCossMS');
  }else if ($argv[1] == 'KBRO2'){
  	$sosql = "'310','330','410','420','610','810','820'";
    $dbh = GetDBH('kbroCossMS');
  }else if ($argv[1] == 'TFM'){
  ...
  $qry2 = "select a.companyno,a.custid,a.subsid from ms0200 a with (nolock),ms0301 b with (nolock) where a.companyno=b.companyno and a.subsid=b.subsid
           ...
           b.chargename like '%上網時間%' and substring(b.sheetstatus, 1, 1) not in ('4', 'A')";
  ...
  $qry3 = "select a.companyno,a.custid,a.subsid from ms0200 a with (nolock),ms3200 b with (nolock) where a.companyno=b.companyno and a.subsid=b.subsid
           and a.companyno in ($sosql) and a.servicename in ('2 CM','5 FTTB','7 EOC') and substring(a.custstatus,1,1) in ('0','1','6','8','9','7','B') and
           b.chargename like '%上網時間%' and recvyn='Y' and passyn = 'N' and billexpire>=getdate() and substring(realrecv,1,1)<>'Y'
	  ...
	  foreach($dbc_result as $item){
		  ...
		  if (empty($total_ms0210[$subsid]) && $subsid != '9061284'){
		    if ($cflag != 'N'){
			  ...
			  $dbc->aws_official_web('update',$query_ary);
			  echo '解除客戶:'.$subsid."<br>\n";
  ...
  //$qry4= "select replace(subsid,' ','') subsid,flag,replace(soid,' ','')soid from cmtime with (nolock) where replace(soid,' ','') in ($sosql)";
  //print $qry4."<br>\n";
  //$sth4 = mssql_query($qry4,$wwwdbh);
  /*while($row = mssql_fetch_assoc($sth4)){
    $subsid = $row['subsid'];
    $cflag = $row['flag'];
    $soid  = $row['soid'];
    ##為避免so為空值, 再查詢一次
    if (empty($soid)){
      $qry = "select companyno,subsid from ms0200 with(Nolock) where subsid='$subsid'";
      $qrysth = mssql_query($qry,$kbro_coss);
      while($qryrow = mssql_fetch_assoc($qrysth)){
         $soid = $qryrow['companyno'];
      }
    }
  ...
  $idx = 0;
  $cm_set = array();
  $qry = "select trim(subsid)subsid,trim(soid)soid,cmset,type,stopyn from cm_set where trim(soid) in ($sosql) ";
  ...
  while($row = oci_fetch_assoc($qrysth)){
    ...
    if (empty($total_ms0210[$subsid])){
      ...
      $upt = "update cm_status set redirect_block='N' where subsid='$subsid' and companyno='$soid'";
      #print $upt."<br>\n";
      #$uptsth2 = oci_parse($dbh_coss,$upt);
      #oci_execute($uptsth2);
      
      if (!empty($iscAry[$soid])){
