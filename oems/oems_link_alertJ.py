        ...
        if  companyno and netid and alert_cnt>0: # for non LS
            ...
            for kk, vv in service_count.items():
                ...
                if kk == '3 DSTB':
                   ...
                   BB_impact_cnt=vv;
            impactType=3116
            if DTV_impact_cnt == 0 and BB_impact_cnt > 0:
               impactType=3114
            elif DTV_impact_cnt > 0 and BB_impact_cnt == 0:
               impactType=3115
            else:
               impactType=3116
            ...
            if  alertType == 'alertLink':
                ...
                oraqrysql = "select sid from oems_tickets_main where ((type='%d' and subtype in (310701,310702,310703)) or ticket_id is not null) and status<=5104 and (impact_list='%s' or impact_list='%s') and account='COSSv2'" % (impactType,impact_str, impact_str2)
            else:
                subtype = 310701
                ...
                oraqrysql = "select sid from oems_tickets_main where ((type='%d' and subtype in (310701,310702,310703)) or ticket_id is not null) and status<=5104 and impact_list='%s' and account='COSSv2'" % (impactType,impact_str)
            print oraqrysql
            ...
            if rs != None and len(rs) > 0:
                ...
                oraupdsql = "insert into oems_tickets_main(status,type,subtype,reason,descr,create_date,operator,account,normal_flag,impact_list,location) \
                             values('5100','%d','%d','%s','%s',sysdate,'%s','COSSv2','C','%s','%s')" % (impactType,subtype, reason_str, descr_str, so_name[companyno], impact_str, taipower_sid)
                print oraupdsql
                try:
            ...
            if sid == 0:
                ...
                oraqrysql = "select sid from oems_tickets_main where type='%d' and subtype in (310701,310702,310703) and status <= 5104 and impact_list='%s' and account='COSSv2' order by create_date desc" % (impactType,impact_str)
                rs = oracon_oems.execall(oraqrysql)
                if rs != None and len(rs) > 0:
                    for a_row in rs:
                        sid = a_row[0]
            ...
            if rs != None and len(rs) > 0:
                ...
                if rs != None and len(rs) > 0:
                    for a_row in rs:
                        ...
                        break
                oraupdsql = "insert into oems_tickets_main(status,type,subtype,reason,descr,create_date,operator,account,normal_flag,impact_list,location) \
                             values('5100',3110,'%d','%s','%s',sysdate,'%s','COSSv2','C','%s','%s')" % (subtype, reason_str, reason_str, so_name[companyno], impact_str, taipower_sid)
                print oraupdsql
                try:
                    oracon_oems.execone(oraupdsql)
                    oracon_oems.commit()
