...
if re.match(r"^\d{3}$", so) is not None:
    sosql = "'%s'" % (so)
else:
    print 'usage:',sys.argv[0],'CompanyNo'
    sys.exit(0)
...
if so in ['101','103','104','300','701']:
    db_host = 'TFMCossMS'
elif so in ['210','220','230','240','250','260','310','330','410','420','610','810','820']:
    db_host = 'kbroCossMS'
elif so in ['106']:
    db_host = 'CossMS_CG'
else:
...
while 1:
    ...
    if curarr:
        for i in range(0, len(curarr)):
            ...
            tel = ''
            if tel!='' and tel1 is not None and len(tel1)>=5 and len(tel1)<=10 and tel1[:2]<>'09':
                tel = "%s,%s" % (tel, tel1[:10])
            elif tel1 is not None and len(tel1)>=5 and len(tel1)<=10 and tel1[:2]<>'09':
                tel = "%s" % (tel1[:10])
            if tel!='' and tel2 is not None and len(tel2)>=5 and len(tel2)<=10 and tel2[:2]<>'09':
                tel = "%s,%s" % (tel, tel2[:10])
            elif tel2 is not None and len(tel2)>=5 and len(tel2)<=10 and tel2[:2]<>'09':
                tel = "%s" % (tel2[:10])
            if tel!='' and tel3 is not None and len(tel3)>=5 and len(tel3)<=10 and tel3[:2]<>'09':
                tel = "%s,%s" % (tel, tel3[:10])
            elif tel3 is not None and len(tel3)>=5 and len(tel3)<=10 and tel3[:2]<>'09':
                tel = "%s" % (tel3[:10])

            email = curarr[i][11]
            if email is not None and len(email) > 0:
                email = email.lower().strip()
            if email is None or email[:4]=='000@' or '@' not in email or '@tfm' in email or '@kbronet' in email or re.match(r"^.{2,}@[^.]{2,}\.[^.]{2,}", email) is None:
                email = ''
